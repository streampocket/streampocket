...
import { uploadReviewImage } from '../_hooks/useOwnReview'
...
export function ReviewImageUploader({ value, onChange }: ReviewImageUploaderProps) {
      ...
      {value ? (
        <div className="space-y-2">
          <div className="relative aspect-16/10 w-full overflow-hidden rounded-lg border border-border bg-gray-100">
            <Image src={value} alt="첨부 이미지 미리보기" fill sizes="640px" className="object-cover" />
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" type="button" onClick={onPick} disabled={uploading}>
